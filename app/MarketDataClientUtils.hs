...
resStatus r = do return $ codeToStatus s
  where s = Http.getResponseStatusCode r
...
resData r = do
  ...
  where dr = Http.getResponseBody r
...
rqQueryStr ReqParams{query=q} = [("limit", Just l)] where 
...
reqExHandler :: HttpException -> IO DataResponse
reqExHandler err = do 
